...
| `1pixsplit.py` | Two-stage split down to a 5×3 piece |
...
- The `Drop` field order `(height, width, row, col)` — identical across
  `cleanup.py`, `1pixsplit.py` and `chipsetup.py`, and *not* what the vendor
  PDF says.
...
`1pixsplit.py` is **not** the basis for the splitting work
